...
def test_case_normal_model_one_country_predictions():

    player_data, player_stats_data, team_data, match_data = first_data_preparation()
    countries = ['France', ]
    # countries = ['England', ]
    min_date_history = datetime.strptime('2013-04-30', "%Y-%m-%d").date()
    min_date_predictions = datetime.strptime('2016-04-30', "%Y-%m-%d").date()
    ...
    mask_date_history = match_data['date'] >= min_date_history
    match_data = match_data[mask_date_history]
    ...
    mask_date = match_data['date'] >= min_date_predictions
    match_data = match_data[mask_date]

    exp_weight_fct = lambda t1, t2: np.exp(- 0.3 * (t2-t1).days / 365.25)
    model = NormalModel(team_universe, weight_fct=exp_weight_fct)
