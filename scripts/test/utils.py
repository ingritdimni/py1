...
def get_fifa_stats(match, player_stats):
    ...
    for player in players:
        ...
        name = "{}_rating".format(player) # shorter name
        names.append(name)
...
# TODO: very interesting, but can be improved
# TODO: idea: vectorize match, and match history
def get_match_features(match, matches, n_history=10, n_common_history=3):
    ...
    matches_home_team = get_last_matches(matches, date, home_team, x=n_history)
    matches_away_team = get_last_matches(matches, date, away_team, x=n_history)

    # Get last x matches of both teams against each other
    last_matches_against = get_last_matches_against_eachother(matches, date, home_team, away_team, x=n_common_history)
...
def create_feables(matches, fifa, bookkeepers, get_overall=False, horizontal=True, x=10, verbose=True):
    ...
    if verbose:
        print("Match features generated in {:.1f} seconds".format(end - start))
