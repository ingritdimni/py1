...
from data_preparation import simple_data_prep, first_data_preparation, create_dict_involved_teams
...
def end_to_end_test_2():
    ...
    match_data, match_features, match_labels, bkm_quotes = simple_data_prep(verbose=1, fable_observed_matches=40,
                                                                            padding=True, remove_nan=False,
                                                                            fable="match_hist",
                                                                            label_format="hot_vectors")

    matches_to_predict_mask = match_data['id'].isin(id_matches_to_predict)
    X_train, X_val = match_features[~matches_to_predict_mask], match_features[matches_to_predict_mask]
    Y_train, Y_val = match_labels[~matches_to_predict_mask], match_labels[matches_to_predict_mask]
